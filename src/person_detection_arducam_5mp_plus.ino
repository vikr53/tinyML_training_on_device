...
// static FlashIAPBlockDevice bd(0xC1000, 0x6000);
...
void setup() { 
  ...
  /****  Faulty FlashIAPBlockDevice Code (for saving parameters in Flash) *******/
...
void loop() {
  ...
  if (y_pred[0] > y_pred[1]) {
    u8x8.drawString(2,5,"Mask?!");
  } else if (y_pred[1] > y_pred[0]) {
    u8x8.drawString(2,5,"No mask?!");
  }
...
/***** FAULTY Alternative (more hard-coded approach to writing to flash programmatically) ******/
...
//     *(float *)NVMC_ERASEPAGE = (uint32_t)(myFlash);
